...
def solution(table, languages, preference):
    ...
    for i in range(len(tmp)):
        for j in range(len(tmp[i])):
            tmp2.append(tmp[i][j].split(" "))
    print(tmp2)

    calc = [0] * len(tmp2)
    for i in range(len(tmp2)):
        for j in range(len(tmp2[i])):
            for d in range(len(languages)):
                if languages[d] == tmp2[i][j]:
                    calc[i] += preference[d] * (len(tmp2) - j + 1)
    ...
    for i in range(len(calc)):
        if calc[i] > ans:
            ...
            alpha.append(tmp2[i][0])

    alpha.append(tmp2[idx][0])
    # print(alpha)
    alpha = sorted(alpha)
    print(alpha[0])
